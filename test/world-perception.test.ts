...
test('全引擎 V2/检查点：富日志(世界/反馈/反思/tick)经 检查点往返 与 全量重建 逐位一致——所有新派生态都落盘', () => {
  const a = tmpStore(); runTurn(a.s, [curiousGenesis]);
  runTurn(a.s, [{ type: 'RELATIONSHIP_OPENED', source: 'system', relationshipId: 'u_a', occurredAt: at(), payload: { relationshipId: 'u_a', kind: 'human', displayRef: 'A' } }]);
  runTurn(a.s, [{ type: 'CONNECTION_OPENED', source: 'host', relationshipId: 'u_a', occurredAt: at(), payload: { relationshipId: 'u_a', host: { kind: 'http', ref: 'say' } } }]);
  // 消息(记忆/价值/预期) + 世界(兴趣/世界记忆/novelty) + 反馈(连接/skills) + 反思(maturity/价值漂移) + tick(reconsolidation)
  for (let i = 0; i < 6; i++) {
    runTurn(a.s, [{ type: 'MESSAGE_RECEIVED', source: 'external_user', relationshipId: 'u_a', occurredAt: at(), payload: { relationshipId: 'u_a', content: '你值得，说出来，我真心在乎你', channel: 'chat' } }]);
    runTurn(a.s, [worldT(['天文航天', '科学'], { valence: 0.7, arousal: 0.5, relevance: 0.9 }, `天文 ${i}`)]);
    runTurn(a.s, [{ type: 'FEEDBACK_PERCEIVED', source: 'autonomous_loop', occurredAt: at(), payload: { actionKind: 'muse', responseKind: 'reaction', valence: 0.6, fromKind: 'human' } }]);
  }
  runTurn(a.s, [{ type: 'REFLECTION_TRIGGERED', source: 'autonomous_loop', occurredAt: at(), payload: { scope: 'recent', windowFromSeq: 0, windowToSeq: 99 } }]);
  const all = a.s.list();
  const full = stateHash(reconstruct(all));
  const rs = resumeFromCheckpoint(captureCheckpoint(all)); // serialize(Set/Map/skills/interests…) → deserialize
  assert.equal(stateHash(projectState(rs.st, rs.uptoSeq)), full, '检查点往返后必须与全量重建逐位一致（novelty/interests/maturity/skills/feedback 全落盘）');
  // 富快照非空（确实跑出了这些派生态）
  const snap = reconstruct(all);
  assert.ok(snap.interests.length > 0 && snap.skills.length > 0 && snap.maturity > 0, '兴趣/技能/成熟度都确实长出来了');
  a.cleanup();
});
