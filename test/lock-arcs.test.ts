...
test('创建生命·显式原型：确定性 + 出生版本不漂移 + 覆盖哈希取型', () => {
  ...
  assert.equal(g1.reconstructVersionAtBirth, 29, '出生版本仍 29（不漂移）');
  ...
  assert.equal(reconstruct(s.list()).reconstructVersion, 29, '重建后版本仍 29');
});
...
test('Arc8 reconstructVersion 标注：快照带版本（支持跨版本切换）', () => {
  ...
  s.append(genesis());
  const snap = reconstruct(s.list());
  assert.equal(snap.reconstructVersion, 29);
