...
test('完整反思树：生成并排序"想要"的目标（确定性）', () => {
  const { path, cleanup } = tmp();
  try {
    const s = boot(path);
    msg(s, '你根本不在乎，都是假的'); // 伤她 → 触发修复/恢复类目标
    const goals = reconstruct(s.list()).goals;
    assert.ok(goals.length > 0, '应生成目标');
    assert.ok(goals.every((g, i) => i === 0 || goals[i - 1].weight >= g.weight), '应按权重排序');
    assert.ok(goals.every((g) => g.intent.length > 0));
  } finally {
    cleanup();
  }
});

test('关系层 ToM：对方模型 + 关系特异的自我（确定性派生）', () => {
  const { path, cleanup } = tmp();
  try {
    const s = boot(path);
    msg(s, '你好，我真心在乎你');
    msg(s, '你根本不在乎，都是假的');
    msg(s, '我真心在乎你，会一直在');
    const b = reconstruct(s.list()).bonds['r_creator'];
    assert.ok(b.theoryOfMind.style.length > 0, '应有对方模型');
    assert.ok(typeof b.relationalSelf.openness === 'number' && b.relationalSelf.stance.length > 0, '应有关系特异的自我');
  } finally {
    cleanup();
  }
});

test('遗忘即抽象：经历压缩成对关系的理解（语义记忆）', () => {
  const { path, cleanup } = tmp();
  try {
    const s = boot(path);
    msg(s, '你好，我真心在乎你');
    msg(s, '你根本不在乎，都是假的');
    msg(s, '我真心在乎你，会一直在');
    const sem = reconstruct(s.list()).semanticMemory.find((x) => x.relationshipId === 'r_creator');
    assert.ok(sem && sem.episodes >= 3 && sem.understanding.length > 0, '应形成对该关系的理解');
    assert.ok(sem.warm >= 1 && sem.conflict >= 1, '应统计出暖与磕碰');
  } finally {
    cleanup();
  }
});
