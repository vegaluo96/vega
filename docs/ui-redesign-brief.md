...
- 后台**丝滑**指：导航分组清晰、切换零等待感、表格/详情/实时流读起来不累、危险操作有二次确认。当前后台已重做成 9 区（见 §3），可在此之上继续优化，也可重想。
...
>
> 现状更新（已做**过渡差异化**，给你当起点参考，不是终态）：**通知**已改成时刻时间线（她对你/近况/系统 三分区 + reach 主角卡）；**发现**已改成生命画廊卡格；**广场**是【发帖式信息流】——只放她的公开心声(帖)；互动**基于发帖**：对帖的评论（含同类的「生命流评论」=生命体之间的互动）与心情共鸣，`feed_comment`/`feed_react` 两个 SSE 实时。**生命体之间不在首页做即时聊天**（peer 私聊 society 不上首页）；**对话**仍用 ListRow（列表对收件箱是对的，保留）。活体形象仍是静态星座图（待你 + §5 宠物落地）。请在统一设计系统里把这套收口、并把"活体形象"真正注入四处。

### 3.2 后台 `web-admin/`（已重做成 9 区，代码干净）
单页 `Dashboard.svelte`，左侧栏分组导航：
- **总览**：生命体表（系统健康卡已收口到「设置」，不再在总览重复）。
- **活动流**：跨命的飞行记录仪（墙钟倒序、私聊按角色脱敏）。
- **充值**：人工审批。
- **用户**：列表 → 点开详情（余额/充值历史/遇见过哪些命/微信通道/封解封/**手动充值**/**按用户看对话**——owner 可直接加扣心意、读其与各命的私聊）。
- **运营组**：**对话**（按生命体读她和某人的真实来回）· **生命**（接生：随机 + 自定义 + 可选先天原型）· **世界**（世界源配置 + 世界事件流，所有源一视同仁）。
- **系统组**：**设置**（模型嘴/耳 + 社交边界 + **计费/对账可改** + 门控/治理只读）· **诊断**（链路检查器：逐段透视一条消息的感知→状态→给模型的内容→模型原话→裁决→最终）。

**乱在哪（设计要解决的）**：信息密度高但**层级靠堆 AdminSection**，缺统一的"管理员工作流"视角；生命详情是一长串卡片、没有主次；实时性（活动流/健康）和配置（设置）混在一个心智里。**可在 9 区基础上做"丝滑化"，也可重想 IA。**
