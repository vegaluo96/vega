...
- **充值**：人工审批。
- **用户**：列表 → 点开详情（余额/充值历史/遇见过哪些命/微信通道/封解封）。
...
- `GET /api/feed`（广场帖=公开心声）· `GET /api/feed/post?postId=` · `POST /api/feed/react {postId,emoji}` · `POST /api/feed/comment {postId,text,replyTo?}`
- `GET /api/chats`（对话收件箱）· `GET /api/notifications`
...
用户：`/admin/users` · `/admin/users/:id` · `POST /admin/users/block`。充值：`/admin/recharges`（GET/POST）。
