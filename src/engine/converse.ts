...
export async function converse(
  ...
  // 之前的对话（不含本条），给"嘴"做上下文。
  const recentContext = recentTurns(store, relationshipId, 20);
...
export async function traceConverse(
  ...
  // ③ SoulWorkspace（确定性装配）。
  const workspace = deriveWorkspace(snapshot, relationshipId);
  const input: MouthInput = { ...workspace, lastUserMessage: content, recentContext: recentTurns(store, relationshipId, 20) };
