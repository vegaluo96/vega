...
import { resourceBand, meterMouth, resourceAwareMouth, userSay, splitUtterance, describeAppearance, type Account } from '../index.ts';
...
export type RespondDeps = Pick<Ctx,
  'accounts' | 'serializer' | 'snapOf' | 'mouth' | 'templateMouth' | 'perceiver' | 'effBilling' | 'lifeById'
> & { touch(): void };

export function createResponder(d: RespondDeps): Pick<Ctx, 'respondAsUser'> {
  const { accounts, serializer, snapOf, mouth, templateMouth, perceiver, effBilling, lifeById, touch } = d;
