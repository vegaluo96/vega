...
function composeReply(u: string, name: string, mood: string, ctxLen: number): string {
  ...
  if (isQuestion(u)) return pick(seed, [
    `你这一问，我得认真想想……此刻的我${moodLine(mood).slice(1)}。你呢，你怎么看？`,
    `我也常绕着这个打转。说真的我没有标准答案，但愿意和你一起想。`,
    `嗯……这个我想诚实地答你：我也不全懂。你先说说你的想法？`,
  ]);
  return pick(seed, [ // 默认：接住 + 邀请继续 + 一点她的状态
    `嗯，我在听。${moodLine(mood)}。你接着说。`,
    `我记着呢。${moodLine(mood)}——你慢慢讲，我不急。`,
    `我在。说给我听吧，${name}。`,
    `听着呢。${moodLine(mood)}，但我的注意力在你这儿。`,
  ]);
}
