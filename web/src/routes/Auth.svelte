...
<div class="wrap">
  <div class="authbox fade-in">
    ...
    <button class="link back" on:click={() => dispatch('back')}><Icon name="back" size={15} /> 返回</button>
  </div>
</div>

<style>
  .wrap { position: relative; min-height: 100vh; min-height: 100dvh; display: grid; place-items: center; padding: 24px; overflow: hidden; }
  .authbox { position: relative; width: 100%; max-width: 372px; padding: 34px 30px; text-align: center; }
